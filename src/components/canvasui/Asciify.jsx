...
export function createAsciify(elements, options = {}) {
  try {
...
function initializeAsciify(elements, options) {
  ...
  function render() {
    ...
    gl.uniform2f(uniforms.uContentOffset, htmlInCanvas
      ? 0
      : (content.scrollLeft - capturedScrollLeft) /
          Math.max(content.clientWidth, 1), htmlInCanvas
      ? 0
  ...
  function onContentScroll() {
    if (htmlInCanvas || destroyed) return;
    window.clearTimeout(fallbackScrollCaptureTimer);
    fallbackScrollCaptureTimer = window.setTimeout(captureFallback, FALLBACK_CAPTURE_DELAY);
    start();
  }
  ...
  return {
    ...
    destroy() {
      ...
      content.removeEventListener("transitioncancel", onFallbackVisualChange, true);
